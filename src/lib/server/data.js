...
export async function allocateOrdinals(env, count, accountId) {
  const results = await env.DB.batch([
    ...
    env.DB.prepare("INSERT INTO usage_events (account_id, n, ts) VALUES (?1, ?2, ?3)").bind(
      accountId,
      count,
      now(),
    ),
...
export async function rateLimit(env, principal) {
  ...
  const res = await env.DB.prepare(
    ...
    .bind(principal, t, RATE_WINDOW_MS)
    .run();
  if (res.meta.changes > 0) return { allowed: true };
  const cur = await env.DB.prepare("SELECT last FROM rate WHERE principal = ?1")
    .bind(principal)
    .first();
...
export async function dispatch(env, op, body = {}) {
  ...
  switch (op) {
    ...
    case "upsert-identity": {
      ...
      let ident = await db
        .prepare("SELECT user_id FROM identities WHERE provider=?1 AND sub=?2")
        .bind(provider, sub)
        .first();
      ...
      if (!ident) {
        ...
        await db.batch([
          db
            .prepare("INSERT INTO users (id,email,name,created) VALUES (?1,?2,?3,?4)")
            .bind(userId, email, name, t),
          db
            .prepare("INSERT INTO identities (provider,sub,user_id) VALUES (?1,?2,?3)")
            .bind(provider, sub, userId),
          db
            .prepare("INSERT INTO accounts (id,name,plan,created) VALUES (?1,?2,'free',?3)")
            .bind(accountId, name ? `${name}'s Team` : "Personal", t),
          db
            .prepare(
              "INSERT INTO memberships (account_id,user_id,role,joined) VALUES (?1,?2,'owner',?3)",
            )
            .bind(accountId, userId, t),
        ]);
      } else {
        userId = ident.user_id;
        const m = await db
          .prepare("SELECT account_id FROM memberships WHERE user_id=?1 ORDER BY joined LIMIT 1")
          .bind(userId)
          .first();
      ...
      await db
        .prepare(
          "INSERT INTO sessions (session_hash,user_id,active_account_id,exp) VALUES (?1,?2,?3,?4)",
        )
        .bind(await sha256hex(session), userId, accountId, now() + SESSION_TTL_MS)
        .run();
    ...
    case "session": {
      const row = await db
        ...
        .bind(await sha256hex(body.session || ""))
        .first();
    ...
    case "create-account": {
      ...
      await db.batch([
        db
          .prepare("INSERT INTO accounts (id,name,plan,created) VALUES (?1,?2,'free',?3)")
          .bind(accountId, name || "New Account", t),
        db
          .prepare(
            "INSERT INTO memberships (account_id,user_id,role,joined) VALUES (?1,?2,'owner',?3)",
          )
          .bind(accountId, user_id, t),
      ]);
    ...
    case "switch-account": {
      ...
      const member = await db
        .prepare("SELECT 1 FROM memberships WHERE account_id=?1 AND user_id=?2")
        .bind(account_id, user_id)
        .first();
      if (!member) return err("not_a_member", 403);
      await db
        .prepare("UPDATE sessions SET active_account_id=?1 WHERE session_hash=?2")
        .bind(account_id, await sha256hex(session))
        .run();
    ...
    case "team-settings": {
      ...
      const me = await db
        .prepare("SELECT role FROM memberships WHERE account_id=?1 AND user_id=?2")
        .bind(body.account_id, body.actor_user_id)
        .first();
      if (!me) return err("forbidden", 403);
      const acct = await db
        .prepare("SELECT join_code FROM accounts WHERE id=?1")
        .bind(body.account_id)
        .first();
      return ok({
        role: me.role,
        join_code: me.role === "owner" ? (acct?.join_code ?? null) : null,
      });
    }
    ...
    case "rotate-join-code": {
      ...
      await db
        .prepare("UPDATE accounts SET join_code=?1 WHERE id=?2")
        .bind(code, body.account_id)
        .run();
    ...
    case "revoke-join-code": {
      ...
      if (me?.role !== "owner") return err("forbidden", 403, { detail: "owners only" });
      await db
        .prepare("UPDATE accounts SET join_code=NULL WHERE id=?1")
        .bind(body.account_id)
        .run();
    ...
    case "join": {
      ...
      const acct = body.code
        ? await db.prepare("SELECT id FROM accounts WHERE join_code=?1").bind(body.code).first()
        : null;
      if (!acct)
        return err("invalid_or_disabled_code", 400, {
          message: "That join code is invalid, was rotated, or invites are disabled.",
        });
      await db
        .prepare(
          "INSERT OR IGNORE INTO memberships (account_id,user_id,role,joined) VALUES (?1,?2,'member',?3)",
        )
        .bind(acct.id, body.user_id, now())
        .run();
    ...
    case "create-key": {
      ...
      await db
        .prepare(
          "INSERT INTO api_keys (key_hash,id,account_id,created_by,label,hint,created) VALUES (?1,?2,?3,?4,?5,?6,?7)",
        )
        .bind(
      ...
      return ok({
        ...
        label: label || "default",
        account_id,
        message: "Shown once. Hashed at rest. This is what the SDKs send.",
      });
    }
    ...
    case "revoke-key": {
      ...
      if (!member) return err("forbidden", 403);
      await db
        .prepare("DELETE FROM api_keys WHERE account_id=?1 AND id=?2")
        .bind(body.account_id, body.key_id)
        .run();
    ...
    case "register-client": {
      ...
      if (!Array.isArray(redirect_uris) || redirect_uris.length === 0)
        return err("invalid_redirect_uris", 400);
      const clientId = rand("client_", 16);
      const clientSecret = rand("secret_", 32);
      await db
        .prepare(
          "INSERT INTO oauth_clients (client_id,name,secret_hash,redirect_uris) VALUES (?1,?2,?3,?4)",
        )
        .bind(
          clientId,
    ...
    case "issue-code": {
      const { session, client_id, redirect_uri, scope, code_challenge, code_challenge_method } =
        body;
      const sess = await db
        .prepare("SELECT active_account_id, exp FROM sessions WHERE session_hash=?1")
        .bind(await sha256hex(session || ""))
        .first();
      if (!sess || sess.exp < now())
        return err("access_denied", 401, { detail: "must be logged in" });
      const client = await db
        .prepare("SELECT redirect_uris FROM oauth_clients WHERE client_id=?1")
        .bind(client_id)
        .first();
      if (!client) return err("invalid_client", 400);
      if (!JSON.parse(client.redirect_uris).includes(redirect_uri))
        return err("invalid_redirect_uri", 400);
      const granted = (scope || SCOPES.join(" "))
        .split(/\s+/)
    ...
    case "token": {
      ...
      if (g === "authorization_code") {
        ...
        if (!(await pkceMatches(body.code_verifier, rec.code_challenge, rec.code_challenge_method)))
          return err("invalid_grant", 400, { detail: "PKCE failed" });
        // record the standing grant so the team can see/revoke this app later
        await db
          .prepare(
            "INSERT INTO oauth_grants (account_id,client_id,scope,created) VALUES (?1,?2,?3,?4) ON CONFLICT(account_id,client_id) DO UPDATE SET scope=?3",
          )
          .bind(rec.account_id, rec.client_id, rec.scope, now())
          .run();
      ...
      if (g === "refresh_token") {
        const rec = await db
          .prepare(
            "SELECT account_id,client_id,scope FROM oauth_tokens WHERE token_hash=?1 AND kind='refresh'",
          )
          .bind(await sha256hex(body.refresh_token || ""))
          .first();
      ...
      if (g === "client_credentials") {
        const client = await db
          .prepare("SELECT secret_hash FROM oauth_clients WHERE client_id=?1")
          .bind(body.client_id || "")
          .first();
        if (!client || client.secret_hash !== (await sha256hex(body.client_secret || "")))
          return err("invalid_client", 401);
        const granted = (body.scope || SCOPES.join(" "))
          .split(/\s+/)
          .filter((x) => SCOPES.includes(x))
          .join(" ");
    ...
    case "revoke-grant": {
      ...
      await db.batch([
        db
          .prepare("DELETE FROM oauth_grants WHERE account_id=?1 AND client_id=?2")
          .bind(body.account_id, body.client_id),
        db
          .prepare("DELETE FROM oauth_tokens WHERE account_id=?1 AND client_id=?2")
          .bind(body.account_id, body.client_id),
      ]);
    ...
    case "verify-token": {
      ...
      const rec = await db
        .prepare(
          "SELECT account_id,scope,exp FROM oauth_tokens WHERE token_hash=?1 AND kind='access'",
        )
        .bind(h)
        .first();
      if (!rec) return err("invalid_token", 401);
      if (rec.exp < now()) {
        await db.prepare("DELETE FROM oauth_tokens WHERE token_hash=?1").bind(h).run();
        return err("expired_token", 401);
      }
      if (body.scope_required && !rec.scope.split(" ").includes(body.scope_required))
        return err("insufficient_scope", 403, { scope_required: body.scope_required });
    ...
    case "quota": {
      ...
      const account = body.account_id.startsWith("team_")
        ? await db.prepare("SELECT plan FROM accounts WHERE id=?1").bind(body.account_id).first()
        : null;
      ...
      const row = await db
        .prepare(
          "SELECT COALESCE(SUM(n),0) AS used FROM usage_events WHERE account_id=?1 AND ts>=?2",
        )
        .bind(body.account_id, dayStart)
        .first();
      ...
      if (limit !== null && used + want > limit)
        return err("quota_exceeded", 402, {
          ...
          limit,
          plan,
          message: `Free tier is ${FREE_DAILY_QUOTA} numbers/day. Upgrade to count higher.`,
        });
    ...
    case "list-usage": {
      ...
      const { results } = await db
        ...
        .bind(size, body.account_id, since)
        .all();
      const tot = await db
        .prepare("SELECT COALESCE(SUM(n),0) AS total FROM usage_events WHERE account_id=?1")
        .bind(body.account_id)
        .first();
      return ok({
        bucket: b,
        window,
        points: results.map((r) => ({ t: Number(r.t) * size, count: Number(r.count) })),
...
async function mintTokens(db, accountId, scope, withRefresh, clientId = null) {
  ...
  await db
    .prepare(
      "INSERT INTO oauth_tokens (token_hash,account_id,client_id,scope,exp,kind) VALUES (?1,?2,?3,?4,?5,'access')",
    )
    .bind(await sha256hex(accessToken), accountId, clientId, scope, now() + TOKEN_TTL_MS)
    .run();
  const out = {
  ...
  if (withRefresh) {
    ...
    await db
      .prepare(
        "INSERT INTO oauth_tokens (token_hash,account_id,client_id,scope,exp,kind) VALUES (?1,?2,?3,?4,?5,'refresh')",
      )
      .bind(await sha256hex(refreshToken), accountId, clientId, scope, now() + 90 * DAY_MS)
      .run();
