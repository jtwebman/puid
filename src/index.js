// index.js — the PUID Worker. Two surfaces on one Worker:
//   puid.dev        marketing site + web UI (login, dashboard, interactive docs)
//   puid.dev/api    the JSON API (ids, ordinal, metrics, oauth2, dashboard AJAX)
//
// Browser-facing auth flows (Google/Microsoft login redirects, the OAuth2 consent
// screen) live at the root because they render/redirect in a browser. Everything
// machine-readable lives under /api. Storage is D1; the only Durable thing here is
// the joke.
...
import { landingPage, docsPage, dashboardPage, upgradePage } from "./site.js";
...
// Secure is omitted over plain http so the real Google/Microsoft OAuth flow (and
// the session cookie) work on http://localhost:<port> during `wrangler dev`.
// In production (https) Secure is always set.
...
// Allocate EXACTLY the ordinals we emit — one atomic D1 allocation per request,
// no per-isolate buffering, so no ordinal is ever stranded in a dead isolate.
// Zero waste, zero gaps. Affordable because requests are capped at 1/sec/account.
...
export default {
  async fetch(request, env) {
    ...
    // ============ SITE (puid.dev) ============
    if (path === "/" || path === "/docs" || path === "/dashboard" || path === "/upgrade") {
      const { locale, persist } = pickLocale(request, url);
      const body = path === "/docs" ? docsPage(locale)
        : path === "/dashboard" ? dashboardPage(locale)
        : path === "/upgrade" ? upgradePage(locale)
        : landingPage(locale);
      const headers = { "content-type": "text/html; charset=utf-8" };
      if (persist) headers["set-cookie"] = `lang=${locale}; Path=/; Max-Age=31536000; SameSite=Lax`;
      return new Response(body, { headers });
    }

    // ---- browser auth flows (Google / Microsoft login) ----
    if (path.startsWith("/auth/login/")) {
      ...
      return new Response(null, { status: 302, headers: {
        ...
        "set-cookie": setCookie("puid_login", JSON.stringify({ state, provider, next }), 600),
      }});
    }
    if (path.startsWith("/auth/callback/")) {
      ...
      return new Response(null, { status: 302, headers: {
        ...
        "set-cookie": setCookie("puid_session", data.session, 60 * 60 * 24 * 30),
      }});
    }
    // ---- TEST-ONLY login bypass ----
    // Lets Playwright simulate "signed in via Google/Microsoft" without driving a
    // real provider (Google blocks automation). Gated by ALLOW_DEV_LOGIN, which is
    // set only in local .dev.vars and NEVER in production. If the flag is unset
    // this route does not exist (404), exactly like any other unknown path.
    if (path === "/auth/dev-login" && env.ALLOW_DEV_LOGIN === "1") {
      const email = url.searchParams.get("email") || "dev@example.com";
      const next = url.searchParams.get("next") || "/dashboard";
      const { data } = await authCall(env, "upsert-identity", {
        provider: "dev", sub: url.searchParams.get("sub") || email, email, name: email.split("@")[0],
      });
      return new Response(null, { status: 302, headers: {
        location: next,
        "set-cookie": setCookie("puid_session", data.session, 60 * 60 * 24 * 30),
    ...
    // ---- OAuth2 consent screen (browser; we are the provider) ----
    ...
    // ---- join an account via a reusable code (browser link) ----
    // puid.dev/join/<code> — anyone with a live code can join. If not signed in,
    // they sign in with Google/Microsoft first (next brings them back here).
...
async function handleApi(request, env, origin, p, url) {
  ...
  if (p === "/.well-known/oauth-authorization-server") {
    return json({
      ...
      scopes_supported: ["puid:generate", "puid:ordinal"],
  ...
  // ---- OAuth2 provider (machine endpoints) ----
  if (p === "/oauth/register" && request.method === "POST") {
    const body = await request.json().catch(() => ({}));
    const { status, data } = await authCall(env, "register-client", body);
  ...
  // ---- dashboard AJAX (session-cookie auth) ----
  ...
  if (p === "/account/create" && request.method === "POST") {
    ...
    if (!s) return json({ error: "not_logged_in" }, 401);
    const body = await request.json().catch(() => ({}));
    const { data } = await authCall(env, "create-account", { user_id: s.user_id, name: body.name });
    return json(data);
  }
  if (p === "/account/switch" && request.method === "POST") {
    ...
    if (!s) return json({ error: "not_logged_in" }, 401);
    const body = await request.json().catch(() => ({}));
    const { status, data } = await authCall(env, "switch-account", { session: s.session, user_id: s.user_id, account_id: body.account_id });
    return json(data, status);
  }
  if (p === "/team/keys" && request.method === "POST") {
    ...
    if (!s) return json({ error: "not_logged_in" }, 401);
    const body = await request.json().catch(() => ({}));
    const { data } = await authCall(env, "create-key", { account_id: s.active_account_id, actor_user_id: s.user_id, label: body.label });
    return json(data);
  }
  if (p === "/team/settings") {
    const s = await needSession();
...
<p>It is requesting permission to:</p>
...
<p style="color:#888;font-size:.85rem">You are authorizing access to a service that returns integers, one per second.</p>
</body>`;
}
