...
async function principalFromRequest(env, request) {
  ...
  const apiKey =
    request.headers.get("x-api-key") || (bearer?.startsWith("puid_live_") ? bearer : null);
  if (apiKey) {
  ...
  if (bearer?.startsWith("puid_at_")) {
    const { status, data } = await authCall(env, "verify-token", {
      token: bearer,
      scope_required: "puid:generate",
    });
...
export async function handle(request, env, url) {
  ...
  if (path.startsWith("/auth/login/")) {
    ...
    if (!PROVIDERS[provider])
      return json({ error: "unknown_provider", supported: Object.keys(PROVIDERS) }, 404);
    if (!env[PROVIDERS[provider].clientIdEnv])
      return json({ error: "provider_not_configured", provider }, 404);
    ...
    return new Response(null, {
      ...
      headers: {
        ...
        "set-cookie": setCookie(
          "puid_login",
          JSON.stringify({ state, provider, next }),
          600,
          secure,
        ),
      },
  ...
  if (path.startsWith("/auth/callback/")) {
    ...
    const saved = (() => {
      try {
        return JSON.parse(cookies(request)["puid_login"] || "{}");
      } catch {
        return {};
      }
    })();
    if (!code || !state || state !== saved.state || provider !== saved.provider)
      return json({ error: "bad_login_state" }, 400);
    let profile;
    try {
      profile = await exchangeAndProfile(provider, env, origin, code);
    } catch (e) {
      return json({ error: "login_failed", detail: e.message }, 502);
    }
  ...
  if (path === "/auth/dev-login" && env.ALLOW_DEV_LOGIN === "1") {
    ...
    const { data } = await authCall(env, "upsert-identity", {
      provider: "dev",
      sub: url.searchParams.get("sub") || email,
      email,
      name: email.split("@")[0],
    });
    return new Response(null, {
  ...
  if (path === "/oauth/authorize") {
    ...
    if (!s)
      return new Response(null, {
        status: 302,
        headers: {
          location: `${origin}/auth/login/google?next=${encodeURIComponent(url.pathname + url.search)}`,
        },
      });
    return new Response(consentPage(Object.fromEntries(url.searchParams), s), {
  ...
  if (path === "/oauth/decision" && request.method === "POST") {
    ...
    if (form.decision !== "approve")
      return new Response(null, {
        status: 302,
        headers: { location: `${form.redirect_uri}?error=access_denied&state=${form.state || ""}` },
      });
    ...
    return new Response(null, {
      status: 302,
      headers: {
        location: `${form.redirect_uri}${sep}code=${data.code}&state=${form.state || ""}`,
      },
    });
  }
  if (path.startsWith("/join/")) {
    ...
    if (!s)
      return new Response(null, {
        status: 302,
        headers: {
          location: `${origin}/auth/login/google?next=${encodeURIComponent(url.pathname)}`,
        },
      });
    const { status, data } = await authCall(env, "join", { code, user_id: s.user_id });
    const dest =
      status === 200 ? `/dashboard?joined=${data.account_id}` : `/dashboard?join_error=1`;
...
async function handleApi(request, env, origin, p, url) {
  ...
  if (p === "/openapi.yaml")
    return new Response(toYaml(specWithBase(baseUrl)), {
  ...
  if (p === "/v1/ids") {
    ...
    if (!principal)
      return json(
        {
          error: "unauthorized",
          message: "Send X-API-Key or a Bearer token. Sign in at /dashboard to get one.",
        },
        401,
      );
    ...
    if (!rl.allowed)
      return json(
        {
          error: "rate_limited",
          message: "One request per second. Want more? That requires an upgrade.",
          retryAfterMs: rl.retryAfterMs,
          upgrade_url: `${origin}/upgrade`,
        },
        429,
        { "retry-after": String(Math.ceil((rl.retryAfterMs || 1000) / 1000)) },
      );
    let n = Number(url.searchParams.get("n") ?? "1");
    if (!Number.isInteger(n) || n < 1 || n > MAX_PER_REQUEST)
      return json({ error: "bad_request", message: `n must be 1..${MAX_PER_REQUEST}.` }, 400);
  ...
  if (p.startsWith("/v1/ordinal/")) {
    const principal = await principalFromRequest(env, request);
    if (!principal)
      return json({ error: "unauthorized", message: "Send X-API-Key or a Bearer token." }, 401);
    const puid = decodeURIComponent(p.split("/").pop());
    try {
      const ordinal = decodePuid(puid);
      return json({ puid, ordinal: ordinal.toString() });
    } catch (e) {
      return json({ error: "bad_request", message: `Not a valid PUID: ${e.message}` }, 400);
    }
  }
  ...
  if (p === "/oauth/register" && request.method === "POST") {
    const { status, data } = await authCall(
      env,
      "register-client",
...
async function handleDashboard(request, env, p, url) {
  ...
  if (p === "/usage") {
    const s = await needSession();
    if (!s) return json({ error: "not_logged_in" }, 401);
    const { data } = await authCall(env, "list-usage", {
  ...
  if (p === "/dev/seed-usage" && env.ALLOW_DEV_LOGIN === "1") {
    ...
    await env.DB.prepare("INSERT INTO usage_events (account_id, n, ts) VALUES (?1, ?2, ?3)")
      .bind(s.active_account_id, want, Date.now())
      .run();
  ...
  if (p === "/account/create" && request.method === "POST") {
    const s = await needSession();
    if (!s) return json({ error: "not_logged_in" }, 401);
    const { data } = await authCall(env, "create-account", {
      user_id: s.user_id,
      name: (await request.json().catch(() => ({}))).name,
    });
    return json(data);
  }
  if (p === "/account/switch" && request.method === "POST") {
    const s = await needSession();
    if (!s) return json({ error: "not_logged_in" }, 401);
    const { status, data } = await authCall(env, "switch-account", {
      session: s.session,
      user_id: s.user_id,
      account_id: (await request.json().catch(() => ({}))).account_id,
    });
    return json(data, status);
  }
  if (p === "/keys") {
  ...
  if (p === "/keys/revoke" && request.method === "POST") {
    const s = await needSession();
    if (!s) return json({ error: "not_logged_in" }, 401);
    const { status, data } = await authCall(env, "revoke-key", {
      account_id: s.active_account_id,
      actor_user_id: s.user_id,
      key_id: (await request.json().catch(() => ({}))).key_id,
    });
    return json(data, status);
  }
  if (p === "/grants") {
  ...
  if (p === "/grants/revoke" && request.method === "POST") {
    const s = await needSession();
    if (!s) return json({ error: "not_logged_in" }, 401);
    const { status, data } = await authCall(env, "revoke-grant", {
      account_id: s.active_account_id,
      actor_user_id: s.user_id,
      client_id: (await request.json().catch(() => ({}))).client_id,
    });
    return json(data, status);
  }
  if (p === "/team/keys" && request.method === "POST") {
    const s = await needSession();
    if (!s) return json({ error: "not_logged_in" }, 401);
    const { data } = await authCall(env, "create-key", {
      account_id: s.active_account_id,
      actor_user_id: s.user_id,
      label: (await request.json().catch(() => ({}))).label,
    });
    return json(data);
  }
  if (p === "/team/settings") {
    const s = await needSession();
    if (!s) return json({ error: "not_logged_in" }, 401);
    const { status, data } = await authCall(env, "team-settings", {
      account_id: s.active_account_id,
      actor_user_id: s.user_id,
    });
    return json(data, status);
  ...
  if (p === "/team/join-code/rotate" && request.method === "POST") {
    const s = await needSession();
    if (!s) return json({ error: "not_logged_in" }, 401);
    const { status, data } = await authCall(env, "rotate-join-code", {
      account_id: s.active_account_id,
      actor_user_id: s.user_id,
    });
    return json(data, status);
  ...
  if (p === "/team/join-code/revoke" && request.method === "POST") {
    const s = await needSession();
    if (!s) return json({ error: "not_logged_in" }, 401);
    const { status, data } = await authCall(env, "revoke-join-code", {
      account_id: s.active_account_id,
      actor_user_id: s.user_id,
    });
    return json(data, status);
